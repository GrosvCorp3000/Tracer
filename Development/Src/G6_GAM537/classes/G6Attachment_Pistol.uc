...
class G6Attachment_Pistol extends UTBeamWeaponAttachment;
...
simulated function CheckPoweredUp()
{
	...
	if (Instigator != None && WorldInfo.GRI != None)
	{
		...
		foreach WorldInfo.AllPawns(class'UTPawn', UTP, Instigator.Location, class<G6Weap_Laser>(WeaponClass).default.WeaponLinkDistance)
		{
			if ( UTP != Instigator && !UTP.bNoWeaponFiring && UTP.DrivenVehicle == None && G6Attachment_Laser(UTP.CurrentWeaponAttachment) != None &&
				WorldInfo.GRI.OnSameTeam(Instigator, UTP) && FastTrace(UTP.Location, Instigator.Location) )
			{
				...
				if (!UTP.IsFirstPerson())
				{
					...
					// only create a connection on one side so we don't double up effects
						...
						G6Attachment_Laser(UTP.CurrentWeaponAttachment).Connections.Find('TargetAttachment', self) == INDEX_NONE )
					{
...
defaultproperties
{
	...
	WeaponClass=class'G6Weap_Laser'
